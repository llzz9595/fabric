...
//创建Tracer Span
func CreateSpan(ctx context.Context,funcName ,in,tag string,log string,txId string){
	//新建tracer.span进行拦截
	if parent := opentracing.SpanFromContext(ctx); parent != nil {
		pctx := parent.Context()
		if tracer := opentracing.GlobalTracer(); tracer != nil {
			checkSpan := tracer.StartSpan(funcName, opentracing.ChildOf(pctx))
			checkSpan.SetTag(funcName,tag)
			checkSpan.LogFields(
				openlog.Object("输入", in),
				openlog.String("日志", log),
				openlog.String("交易Hash", txId),
			)
